...
# 创建数据集实例
data_dir = r"D:\zly\data\WHU_clip/train_clip_1"
transform = transforms.ToTensor()
dataset = ChangeDetectionDataset(data_dir, transform=transform)
dataloader = DataLoader(dataset, batch_size=1, collate_fn=dataset.collate_fn)
...
with open(r"D:\zly\daima\sample\gongkai\result/results_classcvaps_1_min_max.txt", "w") as f:
...
with open(r"D:\zly\daima\sample\gongkai\result/results_classcvaps_0_max_min.txt", "w") as f:
    for index, result in sorted_results:
        f.write(f"Image {index}: OA = {result}\n")
